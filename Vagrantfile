...
private_key_path = ENV['HOME'] + "/hadoop-ansible/temp/kluczyki.pub"
config_data = YAML.load_file("defaults/main.yml")
...
Vagrant.configure("2") do |config|
  (1..config_data['datanode_count']).each do |i|
    config.vm.define "dn#{i}" do |datanode|
      ...
      datanode.vm.provision "file", source: private_key_path, destination: "~/.ssh/kluczyki.pub"
  ...
  config.vm.define "nn1" do |namenode|
    ...
    namenode.vm.provision "file", source: private_key_path, destination: "~/.ssh/kluczyki.pub"
